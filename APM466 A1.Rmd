...
for (i in c(1:11))  # ith bond
{
  for (j in c(1:11)){   # jth Dates
    ...
    ytm = bond.yield(Dates[j],Clean11$maturity.date[i],coupon=Clean11$coupon[i],freq =2,price = Clean11[i,7+j],comp.freq = 2)
    ytms[i,j] = ytm
    }}
```
...
for (i in c(1:11)){
  P = Dirty11[1,6+i]  ## Price of the bond with the earlist maturity date
  FV = 100 + Dirty11$coupon[1] * 100 / 2   ## Future payment at maturity date
  ttm = 2 * Dirty11$month_to_maturity[1] / 12  ## 2 * t with unit year
  spot =2 * ((FV / P)^(1/ttm) -1)  ## r/2 = (FV / P)^(1 / 2t) - 1 (since there is no coupon payment before maturity date)
...
for (i in c(2:11)){   # ith bond
  for(j in c(1:11)){  # jth date
    P = Dirty11[i,6+j]  ## Price of the bond with the earlist maturity date
    coupon = Dirty11$coupon[i] * 100 / 2  ## coupon payment
    ttm = 2 * Dirty11$month_to_maturity[i]/12  ## 2 * t with unit year
    ...
    for (m in c(1:length(coupon_times))){
      PV_coupon = coupon / (1+spots[m,j]/2)^(2*coupon_times[m]) # Present value of the coupons
      rest_P = rest_P - PV_coupon
    }
    FV = 100 + coupon  ## Future payment at the maturity date
    spot =2 * ((FV / rest_P)^(1/ttm) -1)  ## r/2 = (FV / rest_P)^(1 / 2t) - 1 (since there is no coupon payment before maturity date)
    spots[i,j] = spot
  }
}
```
...
# Plot Spots curves
x = c(6,12,18,24,30,36,42,48,54,60)
approx_spots = as.data.frame(approx_spots)
dev.new(width=10, height=6)
plot(x,approx_spots$`Spot 2022/01/10`, type="l", col="green", lwd=1, xlab="months" ,ylab="spot rate",ylim=c(0.006,0.02))
lines(x, approx_spots$`Spot 2022/01/11`, col="red", lwd=2)
lines(x, approx_spots$`Spot 2022/01/12`, col="blue", lwd=2)
lines(x, approx_spots$`Spot 2022/01/13`, col="yellow", lwd=2)
lines(x, approx_spots$`Spot 2022/01/14`, col="pink", lwd=2)
lines(x, approx_spots$`Spot 2022/01/17`, col="grey", lwd=2)
lines(x, approx_spots$`Spot 2022/01/18`, col="cyan", lwd=2)
lines(x, approx_spots$`Spot 2022/01/19`, col="black", lwd=2)
lines(x, approx_spots$`Spot 2022/01/20`, col="orange", lwd=2)
lines(x, approx_spots$`Spot 2022/01/21`, col="darkred", lwd=2)
lines(x, approx_spots$`Spot 2022/01/24`, col="royalblue", lwd=2)
legend(50,0.015,legend = Dates, col=c("green","red","blue","yellow","pink","grey","cyan","black","orange","darkred","royalblue"),lty=1:2, cex=0.8)
```
...
for (i in 1:4){
  for (j in 1:11){
    ...
    ## f_1i = (((1+s_i/2)^(2i)/(1+s_1/2)^2)^(1/(i-1))-1
    one_year = (1 + approx_spots[2,j]/2)^2
    forward[i,j] = (((approx_spots[2*(i+1),j]/2+1)^(2*(i+1)))/one_year)^(1/(2*i))-1
  }
}
...
x = c(1,2,3,4)
dev.new(width=10, height=6)
plot(x,forward$`Forward 2022/01/10`, type="l", col="green", lwd=1, xlab="1yr-(x)yr forward rate" ,ylab="forward rate",ylim = c(0.003,0.0092))
...
legend(3.4,0.007,legend = Dates, col=c("green","red","blue","yellow","pink","grey","cyan","black","orange","darkred","royalblue"),lty=1:2, cex=0.8)
```
